...
기존 결과가 없으면 로그인 후 수준 진단 시작 버튼을 활성화합니다. 버튼을 눌러야 질문이 시작됩니다. 결과가 있으면 최근 결과와 재진단 버튼을 표시합니다. 저장 실패 시 완료라고 표시하지 않으며 재시도할 수 있습니다. 로그아웃과 계정 변경 시 이전 계정의 화면 상태를 지웁니다.
